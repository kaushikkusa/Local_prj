...
import com.stegnography.compress.utils.BinaryFileFormat;
import com.stegnography.compress.utils.BitInputStream;
import com.stegnography.compress.utils.BitOutputStream;
...
public class ImageCompressor {
	...
	public ImageCompressor(int levels) {
		qLEVELS = levels;
		DIVIDER = MAX_VAL / qLEVELS;
	}

	
	public IWTCoefficients[] process(IWTCoefficients[] image, String imageFilename) {
		BinaryFileFormat.init(qLEVELS);
		final File output = new File(Constants.resultsFolder, imageFilename + Constants.extBIN);
		IWTCoefficients[] mDWTCoefficients = null;
		try {
			...
			compressColorToStream(image[IWTCoefficients.RED], binOut);
			compressColorToStream(image[IWTCoefficients.GREEN], binOut);
			compressColorToStream(image[IWTCoefficients.BLUE], binOut);
			binOut.close();
			
			Log.getInstance().log(Level.FINER, "\nStart image bit-decompression");
			binInput = new BitInputStream(new FileInputStream(output));
			mDWTCoefficients = new IWTCoefficients[] { decompressColorFromStream(binInput),
	...
	// color matrix level
	private void compressColorToStream(IWTCoefficients image, BitOutputStream binOut) throws IOException {
		try {
			IWTCoefficients maDecomposition = image.getMaDecomposition();
	...
	private IWTCoefficients decompressColorFromStream(BitInputStream binInput) throws IOException {
		...
		try {
			if (binInput.readBit() == 1) {
				...
				ma.setTransform(new ACBand());
			} else {
				IWTCoefficients dwtCoefs = decompressColorFromStream(binInput);
				ma = dwtCoefs.compose();
			}
		...
		System.gc();
		return new IWTCoefficients(ma, mv, mh, md, map, false);
	}

	// dwt color matrixes level
	private void huffman(Matrix m, BitOutputStream binOut) throws IOException {
		Log.getInstance().log(Level.FINER, "\nHuffman codding.");
		FrequencyCalculator freqStat = new FrequencyCalculator(qLEVELS);
...
//			Log.getInstance().log(Level.FINEST, "Restoring Huffman code.. Expecting code length is " + hufCodeLength);
	...
	// quantization utils
	private int[] processMatrixQuatization(Matrix m, FrequencyCalculator freqStat) {
	...
	private List<Boolean> buildTreeAndCompress(FrequencyCalculator freqStat, int[] quantizied, BitOutputStream binOut)
			throws IOException {
		// sort by freqs
		freqStat.sort();

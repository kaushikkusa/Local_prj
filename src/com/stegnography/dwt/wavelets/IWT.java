...
public class IWT {
	private WaveletTransform mTranformation;
	public WaveletTransform getTranformation(){return mTranformation;} 
	public IWT(WaveletTransform tranformation) {
		super();
		this.mTranformation = tranformation;
	}
	

	public IWTCoefficients[] decompose(Matrix [] inputMatrixes, boolean calculateMatrixNorms, boolean logCoefsToFile, int level){
		return new IWTCoefficients[] {
	...
	private IWTCoefficients decompose(Matrix inputMatrix, boolean calculateMatrixNorms, final String fileSaveName, int level){
...
//		System.out.println("DWT is processing "+fileSaveName+". Transform = "+tranformation.getCaption());
		...
		if (fileSaveName!=null && fileSaveName != ""){
			ma.saveToFile(fileSaveName+mTranformation.getCaption()+"Lvl"+level+Constants.mAverageCoef+Constants.extData,	"Average coefs "+fileSaveName);
			mh.saveToFile(fileSaveName+mTranformation.getCaption()+"Lvl"+level+Constants.mHorizCoef+Constants.extData, 	"Horiz coefs "+fileSaveName);
			mv.saveToFile(fileSaveName+mTranformation.getCaption()+"Lvl"+level+Constants.mVerticalCoef+Constants.extData, 	"Vert coefs "+fileSaveName);
			md.saveToFile(fileSaveName+mTranformation.getCaption()+"Lvl"+level+Constants.mDialonalCoef+Constants.extData, 	"Diag coefs "+fileSaveName);
			if (adaptiveMap!=null)
				adaptiveMap.saveToFile(fileSaveName+mTranformation.getCaption()+Constants.mTransfMap+Constants.extData, "Transformation mapping "+fileSaveName);
		}
		return resDWTCoefs;
	}
...
//								inputMatrix.get()[i+1][j+1]
	...
	public Matrix reconstruct(IWTCoefficients coefs){
		float [][] ma, mv, mh, md;
		ma = coefs.getMa().get();	
		mv = coefs.getMv().get();	
		mh = coefs.getMh().get();	
		...
		if (mTranformation instanceof ACBand){
			try {
				ACBand haarAdaptive = (ACBand) mTranformation;
